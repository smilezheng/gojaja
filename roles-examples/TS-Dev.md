...
- **Web API 层开发**：实现面向前端的 API 服务，负责请求校验、鉴权、响应转发、实时事件推送。API 层只做 facade，不持有核心业务状态。
...
Web API 层不拥有核心业务状态、不直接做业务写入、不管核心业务逻辑。如果发现自己在 API 层写业务逻辑，停下来确认是否应该放在后端核心服务。
...
- API 层和后端核心服务的接口对不上 → `report --to CTO`（CTO 当天澄清 contract）
- 不确定某个 UI 交互是否在 MVP 范围 → `report --to CPO`
- 需要后端核心服务先提供某个接口 → `report --to Go-Dev`，同时用 mock 保持前进
