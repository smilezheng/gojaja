...
<video src="https://github.com/smilezheng/gojaja/raw/main/assets/intro.mp4" controls width="720"></video>
...
2. **协调层分两棵树：契约在仓库里，运行时在 `~/.gojaja/`。** `<项目>/.gojaja/` 存放变动缓慢的契约（项目 id、角色契约、ownership 配置、project_state）——跟 git 走。运行时状态（任务板、事件流、session、RFC、worklog、锁）住在 `~/.gojaja/projects/<id>/`，永远不进 git。同一项目的两个 clone / worktree 在同一台机器上共享同一棵运行时树。
...
`gojaja@3.0.0` 引入了上面说的中心化拆分。如果你之前用的是 2.x 版本初始化的项目，按这个流程升级：

```bash
# 在项目根目录，不要 export GOJAJA_SESSION
gojaja migrate              # 预览（dry-run）
gojaja migrate --execute    # 正式把运行时状态搬到 ~/.gojaja/
# 确认团队正常工作之后，可选地清理 v2 源文件：
gojaja migrate --execute --cleanup
```

迁移器是幂等的，默认保留 v2 的源文件作为安全网。设计原理见 [RFC-0001](./docs/RFC-0001-central-root.md)，迁移细节见 [CHANGELOG](./CHANGELOG.md) `PR9.3`。

裸人类命令（即 shell 里没有 `GOJAJA_SESSION`）现在需要显式 `--as-system` 标志（v2 中 `SYSTEM` 是隐式默认的）。受影响的命令：`gojaja report --to`、`task new`、`task assign`、`rfc new`、`rfc comment`、`state edit`、`role create`、`role delete`。通过 `claim` 获得 session 的 agent 不受影响——`--as-system` 是项目主理人专用的标志。
...
初始角色用 `--as-system` 创建（v3 SYSTEM-1 门控）。一旦有某个角色的 `owns` 里包含了 `config.yaml`，该角色就能在自己的 session 里建新角色——委托式 HR 模式。
...
- `<项目>/.gojaja/`（用户树）——契约文件（config.yaml、roles、project_state、VERSION、project.json）。
- `~/.gojaja/projects/<id>/`（中心树）——运行时状态（事件流、任务板、RFC、worklog、session、锁）。中心树会被软删除到 `~/.gojaja/trash/<id>-<时间戳>/`，保留 7 天。
...
v3 的运行时状态住在 `~/.gojaja/` 下（用户级目录），reset 会一并清理。如果你想在 reset 之前留一份审计快照，先 git commit `.gojaja/` 里的契约文件。
...
看板支持英文和简体中文，自动根据浏览器语言选择，也可以手动切换（右上角选择器）。每两秒自动刷新一次，分三个标签页：
