...
名字的来历："过家家" 是小朋友的角色扮演游戏，每个人分一个家庭角色一起演——这个工具让你的 LLM agent 们在同一个仓库上也玩这个。
...
| `gojaja activate <role> --target X` | 你 | 每个 agent 窗口活配一次角色 |
| 在 `state/project_state.md` 里写产品范围 / 验收标准 | 你 | 项目推进过程中持续维护 |
| 升级工具、重跑 `prompt --write --force-rewrite`、重启窗口 | 你 | CLI 版本变动时 |
| `gojaja claim / plan / ack / wait / report / worklog / task ... / rfc ...` | agent | 每个 turn 自动跑 |
...
# Codex CLI：写到 ~/.codex/skills/gojaja-runtime/
...
角色和窗口绑定，绑定信息不会落到任何项目级文件里。`activate` 命令会打印一段提示词——如果系统支持，自动复制到剪贴板——告诉 agent 怎么领角色、怎么读自己的契约、以及怎么了解 `gojaja` 都能干什么。

```bash
gojaja activate PM      --target cursor   # 粘进 PM 用的 Cursor 窗口
gojaja activate TL      --target claude   # 粘进 TL 用的 Claude 窗口
gojaja activate Backend --target codex    # 粘进 Backend 用的 Codex 窗口
gojaja activate QA      --target cursor   # 另一个 Cursor 窗口，这次是 QA
```
...
`gojaja task status T-0011 Done --force-incomplete`，绕行会作为
事件留在审计流里。
...
gojaja reset --confirm <项目目录名> --purge-codex-skill
```
...
- `<项目>/CLAUDE.md` 里 `<!-- gojaja-runtime:BEGIN ... :END -->` 之间的内容；块外的用户自己写的东西原样保留。如果块就是 CLAUDE.md 的全部内容，整个文件删掉。

用户级的 `~/.codex/skills/gojaja-runtime/` 默认**不动**（跨项目共享）。Reset 也是 "把审计流删干净"的唯一姿势——所有事件都在 `.gojaja/` 下，要保留先 `cp -r .gojaja .gojaja.bak` 或者 git commit 一下。

### 升级 CLI

```bash
npm install -g gojaja@latest
gojaja prompt --target cursor --write --force-rewrite   # 每种工具都重跑一次
...
3. **角色契约还是 TBD 状态**。跑 `gojaja role show <角色>` 看看，如果还是空的，先把 `roles/<id>.md` 填完，再重新粘提示词。

### “两个窗口想认领同一个角色”

第二个窗口会看到 "already claimed by a live session ..."。handbook 教 agent 看到这条要 STOP 然后问你——不要用 `--force`。如果第一个窗口确实死了（你手动关掉的那种），等租约自然过期（默认约 30 分钟），或者在持有 session 的 shell 里手动 `gojaja release <role>`。

### “聊天记录删了，但角色卡住释放不掉”

```bash
unset GOJAJA_SESSION              # 如果还在 shell 里
gojaja release <role>       # 在持有 session 的 shell 里跑，或
# 等约 30 分钟让租约自然过期
```
