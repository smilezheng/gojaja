...
describe("parseArgv — boolean flag whitelist", () => {
  it("regression C-03: --json never consumes the next token as a value", () => {
    // The bug: `agentctl plan --json PM` used to parse as flags.json="PM",
    // positional=[]. Then boolFlag('json') returns false and the role
    // argument is lost, leading to silent MA_SESSION errors.
  ...
  it("mixed: --write before a positional argument still leaves the positional intact", () => {
    // `agentctl prompt --write` historically required no positional, but
    // a user accidentally typing `agentctl activate --write PM --target cursor`
